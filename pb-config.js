...
(function (g) {
  ...
  g.PB_FLAGS = {
    ...
    ,promoCodes: false       // OFF (2026-07-14, build in progress): owner-managed promo codes (Fleet -> Pricing Rules -> Promo Codes). Percent or $ off, per booking-type or ALL, expiry deadline + optional max-uses; discounts SERVICE + ADD-ONS only (never the refundable deposit). Codes publish live to the worker (/promo/publish) and each checkout (portal, booking.live, events) validates via /promo/validate. Additive + reversible: false -> no promo UI, no code fields on any surface. Flip ON per-surface only after the worker is deployed + verified.
  };
